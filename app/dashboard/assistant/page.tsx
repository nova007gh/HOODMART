      ...
      <DashboardLayout>
        <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-400">Ask questions about your business and get AI-powered insights from live data.</p>
          </div>
          <div className="flex items-center gap-2 text-xs text-zinc-500">
            <Clock className="h-4 w-4" />
            <span>{remaining.hour} requests left this hour</span>
          </div>
        </div>
            ...
            <Card className="flex-1 flex flex-col glass-card overflow-hidden">
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                {messages.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-full text-center py-12">
                    ...
                    <h2 className="text-xl font-bold text-white mb-2">Ask EMDPOS Intelligence</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-2xl w-full">
                      {SUGGESTED_QUESTIONS.map((q) => (
                        <button
                          ...
                          className="text-left text-sm px-4 py-3 rounded-lg bg-zinc-900/60 border border-zinc-800 hover:border-yellow-500/40 hover:bg-yellow-500/5 text-zinc-300 transition-colors"
                        >
                          <Zap className="h-3.5 w-3.5 text-yellow-500 inline mr-2" />
                          {q}
                        </button>
                      ))}
                    </div>
                  ...
                  <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[85%] ${msg.role === 'user' ? '' : 'w-full'}`}>
                      {msg.role === 'user' ? (
                        ...
                        <div className={`rounded-lg px-4 py-3 ${msg.error ? 'bg-red-900/20 border border-red-800' : 'bg-zinc-900/60 border border-zinc-800'}`}>
                          {msg.loading ? (
              ...
              <div className="border-t border-zinc-800 p-3 sm:p-4">
                <div className="flex gap-2">
                  ...
                  />
                  {loading ? (
                    <Button onClick={handleStop} variant="outline" className="border-red-500/40 text-red-400 hover:bg-red-500/10">
                      Stop
                    </Button>
                  ) : (
                    <Button
...
function MarkdownContent({ content }: { content: string }) {
  ...
  lines.forEach((line, i) => {
    if (line.startsWith('**') && line.endsWith('**')) {
      elements.push(<p key={i} className="font-bold text-yellow-500 mt-2 mb-1">{line.slice(2, -2)}</p>)
    } else if (line.startsWith('- ')) {
      elements.push(<p key={i} className="text-sm text-zinc-300 ml-4 mb-0.5">• {line.slice(2)}</p>)
    } else if (line.startsWith('  ')) {
      elements.push(<p key={i} className="text-xs text-zinc-400 ml-8 mb-0.5">{line.trim()}</p>)
    } else if (line.match(/^\d+\.\s/)) {
      elements.push(<p key={i} className="text-sm text-zinc-300 ml-4 mb-0.5">{line}</p>)
    } else if (line.trim() === '') {
      ...
      elements.push(<p key={i} className="text-sm text-zinc-200 mb-1">{rendered}</p>)
    }
  })
